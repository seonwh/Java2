...
* 바이트 스트림 
  * 여러 개의 스트림을 연결하여 사용 가능함
* 문자 스트림 - FileReader 클래스
  1. 파일 입력 스트림 생성 : 스트림을 생성하고 파일을 열어 스트림과 연결
  2. 파일 읽기 : read()로 문자 하나 씩 파일에서 읽음
  3. 스트림 닫기 : close()로 더 이상 필요없으면 닫아야 함, 닫힌 스트림에서는 읽을 수 없음.
* 파일 입출력 동안 예외 발생 가능
  * 스트림 생성 동안 : FileNotFoundException 발생 가능
    * 파일 경로명 오류, 디스크 고장 등으로 파일을 열 수 없음
  * 파일 읽기, 쓰기, 닫기를 하는 동안 : IOException 발생 가능
    * 디스크 오동작, 파일이 중간에 깨짐, 디스크 공간이 모자라 파일 입출력 불가
  * try-catch 블록 반드시 필요
* 문자 스트림으로 텍스트 파일 쓰기 - FileWriter 클래스
  1. 파일 출력 스트림 생성 : 스트림 생성 후 파일을 열어 스트림과 연결
  2. 파일 쓰기 : write()로 문자 하나 씩 파일에 기록, 블록 단위로 쓰기 가능
  3. 스트림 닫기 : close()로 스트림 닫기
* 바이트 스트림으로 바이너리 파일 쓰기 - FileOutputStream 클래스
  1. 파일 출력 스트림 생성 : 스트림 생성 후 파일을 열어 스트림과 연결
